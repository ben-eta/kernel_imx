...
#define dev_printk(level, dev, format, arg...)    \ 
    printk(level "%s : " format , dev_driver_string(dev) , ## arg) 
...
#endif
static int
isl1208_i2c_get_int(struct i2c_client *client)
{
        int nint = i2c_smbus_read_byte_data(client, ISL1208_REG_INT);
        if (nint < 0)
                return -EIO;
...
static int isl1208_rtc_alarm_irq_enable(struct device *dev, unsigned int enable)
{
        return 0;
}
...
static const struct rtc_class_ops isl1208_rtc_ops = {
	...
	.alarm_irq_enable = isl1208_rtc_alarm_irq_enable,
};
...
//add ben
static ssize_t
isl1208_sysfs_show_sr(struct device *dev,
                         struct device_attribute *attr, char *buf)
{
        int sr = isl1208_i2c_get_sr(to_i2c_client(dev));
        if (sr < 0)
                return sr;

        return sprintf(buf, "Control and Status:sr=%d\n", sr);
}

static DEVICE_ATTR(sr, S_IRUGO, isl1208_sysfs_show_sr, NULL);

static ssize_t
isl1208_sysfs_show_cint(struct device *dev,
                         struct device_attribute *attr, char *buf)
{
        int nint = isl1208_i2c_get_int(to_i2c_client(dev));
        if (nint < 0)
                return nint;

        return sprintf(buf, "Control and Status:int=%d\n", nint);
}

static DEVICE_ATTR(cint, S_IRUGO, isl1208_sysfs_show_cint, NULL);
...
//ben
#if 0
static ssize_t isl1208_show(struct kobject *kobj, struct kobj_attribute *attr,
    char *buf)
{
...
static ssize_t isl1208_store(struct kobject *kobj, struct kobj_attribute *attr,
      const char *buf, size_t count)
{
...
static struct kobj_attribute isl1208_attribute =
      __ATTR(isl1208_control, 0666, isl1208_show, isl1208_store);

static struct attribute *attrs[] = {
    &isl1208_attribute.attr,
    NULL,
};
#endif
...
isl1208_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//ben
#if 0
	dev_dbg(&client->dev, "isl1208!\n");
    	isl1208_kobj = kobject_create_and_add("isl1208D", kernel_kobj);
    	if (!isl1208_kobj)
        	return -ENOMEM;

    	retval = sysfs_create_group(isl1208_kobj, &attr_group);
    	if (retval)
         	kobject_put(isl1208_kobj);
#endif	
	return 0;
